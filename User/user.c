...
void Test_Debug(void)
{
    ...
    V_ad=(float)V_ad/Jk516save.Debug_Value[4+V_Range].ad_value;
...
const u8 send_dot[]={3,2,1,3,2,4,3,2,4,3,2};
...
//        else
        {
            WriteString_Big ( TESTVALUE_X+32, 160, "------",2 );
            WriteString_Big ( TESTVALUE_X+32, 260, "-------",2 );
...
//    }
   ...
   // LCD_ShowFontCN_40_55(TESTVALUE_X+48*6,200,40,55, (uint8_t*)Out_Assic+14*40*55/8);
...
//    u32 date;
   ...
   Send_To_U.back=9;
    //DispBuf[6]=' ';
        ...
        {
            if(value.polar)
                WriteString_Big ( TESTVALUE_X+32, TESTVALUE_Y, " ",2); 
            else
                WriteString_Big ( TESTVALUE_X+32, TESTVALUE_Y, "-",2 );
              WriteString_Big ( TESTVALUE_X+48+32, TESTVALUE_Y, DispBuf ,2); 
			if(Range <3)
				LCD_ShowFontCN_40_55(TESTVALUE_X+48*7,200-2,40,55,(uint8_t*)Out_Assic+17*40*55/8);//m
			else
				LCD_ShowFontCN_40_55(TESTVALUE_X+48*7,200-2,40,55,(uint8_t*)Out_Assic+22*40*55/8);//m
 ...
 
     {
         ...
         {
             if(value_v.polar)
               WriteString_Big ( TESTVALUE_X-16, TESTVALUE_Y+TEST_VALUE_YMID, " ",2 ); 
           else
               WriteString_Big ( TESTVALUE_X-16, TESTVALUE_Y+TEST_VALUE_YMID, "-",2 );
          WriteString_Big ( TESTVALUE_X+32, TESTVALUE_Y+TEST_VALUE_YMID, DispBuf,2 ); 
         }
    }
...
//			for(i=0;i<4;i++)
//			{
//				 WriteString_16(16+i*128, LCD_HIGH-40, "MEAS DISP",  0);
//			
//			}
            WriteString_16(32, BUTTON_1, "MEAS",  0);
            WriteString_16(32, BUTTON_2, "DISP",  0);
            WriteString_16(32+BUTTON_W, BUTTON_1, "MEAS",  0);
            WriteString_16(32+BUTTON_W, BUTTON_2, "SETUP",  0);
            WriteString_16(32+BUTTON_W*2, BUTTON_1, " SYS",  0);
...
//			for(i=0;i<4;i++)
//			{
//				 WriteString_16(16+i*128, LCD_HIGH-40, "测量显示",  0);
//			
//			}
            WriteString_16(24, BUTTON_1, "测 量",  0);
            WriteString_16(24, BUTTON_2, "显 示",  0);
            WriteString_16(24+BUTTON_W, BUTTON_1, "测 量",  0);
            WriteString_16(24+BUTTON_W, BUTTON_2, "设 置",  0);
            WriteString_16(24+BUTTON_W*2, BUTTON_1, "系 统",  0);
...
void Disp_Test_Item(void)
{
	...
	{
		if(i<3)
			WriteString_16(LIST1, FIRSTLINE+i*HIGH1, pt[i],  0);
		else
			WriteString_16(LIST2, FIRSTLINE+(i-3)*HIGH1, pt[i],  0);
...
//电阻上限
    ...
    LCD_DrawFullRect( DISPX1, FIRSTLINE+HIGH1,RECT , HIGH1  ) ;//SPACE1
	WriteString_16(DISPX1, FIRSTLINE+HIGH1, DispBuf,  0);
    WriteString_16(DISPX1+7*16, FIRSTLINE+HIGH1,DISP_UINT[Jk516save.Set_Data.High_Res.Unit],  0);
...
//电阻下限
    ...
    LCD_DrawFullRect( DISPX1, FIRSTLINE+HIGH1*2,RECT , HIGH1  ) ;
	WriteString_16(DISPX1, FIRSTLINE+HIGH1*2, DispBuf,  0);
    WriteString_16(DISPX1+7*16, FIRSTLINE+HIGH1*2,DISP_UINT[Jk516save.Set_Data.Res_low.Unit],  0);
...
//量程
	...
	LCD_DrawFullRect( DISPX2, FIRSTLINE,SELECT_2END-LIST2-136 , HIGH1) ;//SPACE1
    if(Jk516save.Set_Data.Range_Set==0)
        
        WriteString_16(DISPX2, FIRSTLINE, ppt[0],  0);//待改
    else
        WriteString_16(DISPX2, FIRSTLINE, ppt[Jk516save.Set_Data.Range+1],  0);//待改
...
//电压上限
    ...
    LCD_DrawFullRect( DISPX2, FIRSTLINE+HIGH1,SELECT_2END-LIST2-136 , HIGH1) ;//SPACE1
	WriteString_16(DISPX2, FIRSTLINE+HIGH1*1, DispBuf,  0);
	WriteString_16(DISPX2+16*8, FIRSTLINE+HIGH1, "V",  0);
...
//电压下限
    ...
    LCD_DrawFullRect( DISPX2, FIRSTLINE+HIGH1*2,SELECT_2END-LIST2-136 , HIGH1) ;//SPACE1
	WriteString_16(DISPX2, FIRSTLINE+HIGH1*2, DispBuf,  0);
	WriteString_16(DISPX2+16*8, FIRSTLINE+HIGH1*2, "V",  0);
	Disp_Fastbutton();
	...
	switch(num)
	{
		...
		case 4:
			...
			for(i=0;i<5;i++)
			{
				...
				WriteString_16(16+i*BUTTON_W, BUTTON_1+16, User_Rangeset[i],  0);
			}
			break;
		case 5:
			Colour.Fword=Black;
...
void DispSet_value(u8 keynum)
{
	...
	WriteString_16(DISPX1+16, FIRSTLINE, pt[Jk516save.Set_Data.trip],  0);//增加算法  把顺序改过来
...
//电阻下限	
	...
	Hex_Format(Jk516save.Set_Data.Res_low.Num , Jk516save.Set_Data.Res_low.Dot , 5 , 0);
	LCD_DrawFullRect( DISPX1, FIRSTLINE+HIGH1*4,RECT , HIGH1  ) ;
	WriteString_16(DISPX1, FIRSTLINE+HIGH1*4, DispBuf,  0);
	WriteString_16(DISPX1+16*7, FIRSTLINE+HIGH1*4,DISP_UINT[Jk516save.Set_Data.Res_low.Unit],  0);
...
//电压比较	
	...
	Hex_Format(Jk516save.Set_Data.V_low.Num , Jk516save.Set_Data.V_low.Dot , 6 , 0);
	LCD_DrawFullRect( DISPX1, FIRSTLINE+HIGH1*7,RECT , HIGH1  ) ;
	WriteString_16(DISPX1, FIRSTLINE+HIGH1*7, DispBuf,  0);
	WriteString_16(DISPX1+16*8, FIRSTLINE+HIGH1*7, "V",  0);
...
//讯响
	...
	LCD_DrawFullRect( DISPX2, FIRSTLINE-2+HIGH1*3,RECT, HIGH1  ) ;
	WriteString_16(DISPX2, FIRSTLINE+HIGH1*3, DispBuf,  0);
    WriteString_16(DISPX2+16*7, FIRSTLINE+HIGH1*3+2,DISP_UINT[Jk516save.Set_Data.Nominal_Res.Unit],  0);
...
//电阻上限
	...
	Hex_Format(Jk516save.Set_Data.High_Res.Num , Jk516save.Set_Data.High_Res.Dot , 5 , 0);
	LCD_DrawFullRect( DISPX2, FIRSTLINE+HIGH1*4,RECT, HIGH1 ) ;
	WriteString_16(DISPX2, FIRSTLINE+HIGH1*4, DispBuf,  0);
	WriteString_16(DISPX2+16*7, FIRSTLINE+HIGH1*4,DISP_UINT[Jk516save.Set_Data.High_Res.Unit],  0);
...
//电压标称
	...
	LCD_DrawFullRect( DISPX2, FIRSTLINE+HIGH1*6,RECT , HIGH1) ;
	WriteString_16(DISPX2, FIRSTLINE+HIGH1*6, DispBuf,  0);
	
	WriteString_16(DISPX2+16*8, FIRSTLINE+HIGH1*6, "V",  0);
...
//电压上限
	...
	Hex_Format(Jk516save.Set_Data.V_high.Num, Jk516save.Set_Data.V_high.Dot , 6 , 0);
	LCD_DrawFullRect( DISPX2, FIRSTLINE+HIGH1*7,RECT , HIGH1) ;
	WriteString_16(DISPX2, FIRSTLINE+HIGH1*7, DispBuf,  0);
...
//    keynum=1;
	...
	LCD_DrawFullRect( DISPX1, FIRSTLINE,RECT , HIGH1 ) ;//SPACE1
	WriteString_16(DISPX1, FIRSTLINE, pt[Jk516save.Sys_Setvalue.uart],  0);//
...
//波特率
	...
	LCD_DrawFullRect( DISPX1, FIRSTLINE+HIGH1,RECT , HIGH1) ;//SPACE1
	WriteString_16(DISPX1, FIRSTLINE+HIGH1, Sys_Buard_Value[Jk516save.Sys_Setvalue.buard],  0);
	//U盘开关
    Black_Select=(keynum==3)?1:0;
	if(Black_Select)
	{
	...
	LCD_DrawFullRect( DISPX1, FIRSTLINE+HIGH1*2,RECT , HIGH1) ;//SPACE1
	WriteString_16(DISPX1, FIRSTLINE+HIGH1*2, pt[Jk516save.Sys_Setvalue.u_flag],  0);
	//IO接口开关
    Black_Select=(keynum==4)?1:0;
	if(Black_Select)
	{
	...
	LCD_DrawFullRect( DISPX1, FIRSTLINE+HIGH1*3,RECT , HIGH1) ;//SPACE1
	WriteString_16(DISPX1, FIRSTLINE+HIGH1*3, pt[Jk516save.Sys_Setvalue.plc],  0);
...
//显示语言
	...
	LCD_DrawFullRect( DISPX1, FIRSTLINE+HIGH1*4,RECT , HIGH1) ;//SPACE1
	WriteString_16(DISPX1, FIRSTLINE+HIGH1*4, Sys_Language_Value[Jk516save.Sys_Setvalue.lanage],  0);
...
//年
	...
	LCD_DrawFullRect( DISPX1, FIRSTLINE+HIGH1*5,32 , HIGH1) ;//SPACE1

	sprintf((char *)DispBuf,"%02d",RTC_DateStructure.RTC_Year);
	WriteString_16(DISPX1, FIRSTLINE+HIGH1*5, DispBuf,  0);
	Colour.black=LCD_COLOR_TEST_BACK;
	WriteString_16(DISPX1+32, FIRSTLINE+HIGH1*5, "-",  0);
//月
	...
	LCD_DrawFullRect( DISPX1+48, FIRSTLINE+HIGH1*5,32 , HIGH1 ) ;//SPACE1

	sprintf((char *)DispBuf,"%02d",RTC_DateStructure.RTC_Month);
	WriteString_16(DISPX1+48, FIRSTLINE+HIGH1*5, DispBuf,  0);
	Colour.black=LCD_COLOR_TEST_BACK;
	WriteString_16(DISPX1+80, FIRSTLINE+HIGH1*5, "-",  0);
	...
	LCD_DrawFullRect( DISPX1+96, FIRSTLINE+HIGH1*5,32 ,HIGH1 ) ;//SPACE1

	sprintf((char *)DispBuf,"%02d",RTC_DateStructure.RTC_Date);
	WriteString_16(DISPX1+96, FIRSTLINE+HIGH1*5, DispBuf,  0);
...
//时
	...
	LCD_DrawFullRect( DISPX1, FIRSTLINE+HIGH1*6,32 , HIGH1 ) ;//SPACE1

	sprintf((char *)DispBuf,"%02d",RTC_TimeStructure.RTC_Hours);
	WriteString_16(DISPX1, FIRSTLINE+HIGH1*6, DispBuf,  0);
	Colour.black=LCD_COLOR_TEST_BACK;
	WriteString_16(DISPX1+32, FIRSTLINE+HIGH1*6, ":",  0);
//分
	...
	LCD_DrawFullRect( DISPX1+16*3, FIRSTLINE+HIGH1*6,32 , HIGH1 ) ;//SPACE1

	sprintf((char *)DispBuf,"%02d",RTC_TimeStructure.RTC_Minutes);
	WriteString_16(DISPX1+48, FIRSTLINE+HIGH1*6, DispBuf,  0);
	Colour.black=LCD_COLOR_TEST_BACK;
	WriteString_16(DISPX1+80, FIRSTLINE+HIGH1*6, ":",  0);
	...
	LCD_DrawFullRect( DISPX1+96, FIRSTLINE+HIGH1*6,32 ,HIGH1 ) ;//SPACE1
	sprintf((char *)DispBuf,"%02d",RTC_TimeStructure.RTC_Seconds);
	WriteString_16(DISPX1+96, FIRSTLINE+HIGH1*6, DispBuf,  0);
	//文件名称
   Black_Select=(keynum==12)?1:0;
	if(Black_Select)
	{
	...
	LCD_DrawFullRect( DISPX1, FIRSTLINE+HIGH1*7,RECT ,HIGH1) ;//SPACE1
	WriteString_16(DISPX1, FIRSTLINE+HIGH1*7,(const uint8_t *) Jk516save.Sys_Setvalue.textname,  0);
...
//	WriteString_16(LIST1, FIRSTLINE, User_ListScan_Item[0],  0);
    ...
    WriteString_16(LIST1+32*5, FIRSTLINE+SPACE1*3, (const uint8_t*)Jk516save.fac_num,  0);
//	else
//	{
//		WriteString_16(LIST2,FIRSTLINE+SPACE1*(i-sizeof(Sys_Setitem)/(sizeof(Sys_Setitem[0]))/2), Sys_Setitem[i],  0);
//	}
...
//        hand=1;
    ...
    if(range>6)
        range=6;
    Colour.black =LCD_COLOR_TEST_BACK;
    WriteString_16(LCD_WITH-200, 400-32, Range_Disp_Test[hand][range],  0);
...
//    RTC_TimeStructure.RTC_Seconds);
    ...
    //);
    Colour.black =LCD_COLOR_TEST_BACK;
    WriteString_16(LIST2+160, LIST1+4, (const uint8_t *)LCDTemp,  0);
