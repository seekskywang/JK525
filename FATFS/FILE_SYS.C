...
uint8_t	CH376ReadBlock( PUINT8 buf )  /* 从当前主机端点的接收缓冲区读取数据块,返回长度 */
{
	uint8_t	s, l;
...
uint8_t	CH376WriteReqBlock( PUINT8 buf )  /* 向内部指定缓冲区写入请求的数据块,返回长度 */
{
	uint8_t	s, l;
...
void	CH376WriteHostBlock( PUINT8 buf, uint8_t len )  /* 向USB主机端点的发送缓冲区写入数据块 */
{
...
void	CH376WriteOfsBlock( PUINT8 buf, uint8_t ofs, uint8_t len )  /* 向内部缓冲区指定偏移地址写入数据块 */
{
...
uint8_t	CH376FileOpen( PUINT8 name )  /* 在根目录或者当前目录下打开文件或者目录(文件夹) */
{
...
uint8_t	CH376FileCreate( PUINT8 name )  /* 在根目录或者当前目录下新建文件,如果文件已经存在那么先删除 */
{
	if ( name ) CH376SetFileName( name );  /* 设置将要操作的文件的文件名 */
	return( CH376SendCmdWaitInt( CMD0H_FILE_CREATE ) );
}

uint8_t	CH376DirCreate( PUINT8 name )  /* 在根目录下新建目录(文件夹)并打开,如果目录已经存在那么直接打开 */
{
	CH376SetFileName( name );  /* 设置将要操作的文件的文件名 */
#ifndef	DEF_IC_V43_U
	if ( name[0] == DEF_SEPAR_CHAR1 || name[0] == DEF_SEPAR_CHAR2 ) CH376WriteVar32( VAR_CURRENT_CLUST, 0 );
...
uint8_t	CH376SeparatePath( PUINT8 path )  /* 从路径中分离出最后一级文件名或者目录(文件夹)名,返回最后一级文件名或者目录名的字节偏移 */
{
	PUINT8	pName;
...
uint8_t	CH376FileOpenDir( PUINT8 PathName, uint8_t StopName )  /* 打开多级目录下的文件或者目录的上级目录,支持多级目录路径,支持路径分隔符,路径长度不超过255个字符 */
...
uint8_t	CH376FileOpenPath( PUINT8 PathName )  /* 打开多级目录下的文件或者目录(文件夹),支持多级目录路径,支持路径分隔符,路径长度不超过255个字符 */
{
	return( CH376FileOpenDir( PathName, 0xFF ) );
}

uint8_t	CH376FileCreatePath( PUINT8 PathName )  /* 新建多级目录下的文件,支持多级目录路径,支持路径分隔符,路径长度不超过255个字符 */
{
	uint8_t	s;
	uint8_t	Name;
...
#ifdef	EN_DIR_CREATE
uint8_t	CH376DirCreatePath( PUINT8 PathName )  /* 新建多级目录下的目录(文件夹)并打开,支持多级目录路径,支持路径分隔符,路径长度不超过255个字符 */
{
	uint8_t	s;
	uint8_t	Name;
	uint8_t	ClustBuf[4];
...
uint8_t	CH376FileErase( PUINT8 PathName )  /* 删除文件,如果已经打开则直接删除,否则对于文件会先打开再删除,支持多级目录路径 */
{
	uint8_t	s;
	if ( PathName ) {  /* 文件尚未打开 */
...
uint8_t	CH376ByteRead( PUINT8 buf, UINT16 ReqCount, PUINT16 RealCount )  /* 以字节为单位从当前位置读取数据块 */
{
	uint8_t	s;
...
uint8_t	CH376ByteWrite( PUINT8 buf, UINT16 ReqCount, PUINT16 RealCount )  /* 以字节为单位向当前位置写入数据块 */
{
	uint8_t	s;
...
uint8_t	CH376DiskReadSec( PUINT8 buf, UINT32 iLbaStart, uint8_t iSectorCount )  /* 从U盘读取多个扇区的数据块到缓冲区,不支持SD卡 */
...
uint8_t	CH376DiskWriteSec( PUINT8 buf, UINT32 iLbaStart, uint8_t iSectorCount )  /* 将缓冲区中的多个扇区的数据块写入U盘,不支持SD卡 */
...
uint8_t	CH376SecRead( PUINT8 buf, uint8_t ReqCount, PUINT8 RealCount )  /* 以扇区为单位从当前位置读取数据块,不支持SD卡 */
{
	uint8_t	s;
	uint8_t	cnt;
	UINT32	StaSec;
...
uint8_t	CH376SecWrite( PUINT8 buf, uint8_t ReqCount, PUINT8 RealCount )  /* 以扇区为单位在当前位置写入数据块,不支持SD卡 */
{
	uint8_t	s;
	uint8_t	cnt;
	UINT32	StaSec;
...
uint8_t	CH376LongNameWrite( PUINT8 buf, UINT16 ReqCount )  /* 长文件名专用的字节写子程序 */
{
	uint8_t	s;
#ifndef	DEF_IC_V43_U
	uint8_t	c;
...
uint8_t	CH376CheckNameSum( PUINT8 DirName )  /* 计算长文件名的短文件名检验和,输入为无小数点分隔符的固定11字节格式 */
{
	uint8_t	NameLen;
	uint8_t	CheckSum;
	CheckSum = 0;
...
uint8_t	CH376LocateInUpDir( PUINT8 PathName )  /* 在上级目录(文件夹)中移动文件指针到当前文件目录信息所在的扇区 */
...
uint8_t	CH376GetLongName( PUINT8 PathName, PUINT8 LongName )  /* 由短文件名或者目录(文件夹)名获得相应的长文件名 */
...
uint8_t	CH376CreateLongName( PUINT8 PathName, PUINT8 LongName )  /* 新建具有长文件名的文件,关闭文件后返回,LongName输入路径必须在RAM中 */
