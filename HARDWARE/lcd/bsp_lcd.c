...
//			if(!((t+1)%(LCD_Currentfonts->Width/8)))
//			{
//				Ypos++;
//				Xpos=x;
//				
...
//		if( (((c[index] >>7) - counter)&0x01) == 0x01 )
//      {
...
{                                  
	...
	{        
		for(t=0;t<32*4;t++)
		{   
			temp=GB_32[fno].Msk[t];                //gb16Dot[fno][t];    //调用16*08??   
...
//	
	...
	LCD_ShowFontCN_40_55(TESTVALUE_X+48*8,200-2,40,55,(uint8_t*)Out_Assic+20*40*55/8);//Ω
	//LCD_ShowFontCN_40_55(0,92+55,40,55,(uint8_t*)Out_Assic+27*40*55/8);//V
//	
	LCD_ShowFontCN_40_55(TESTVALUE_X+48*8,300-2,40,55,(uint8_t*)Out_Assic+27*40*55/8);//V
...
//			/*一个汉字两字节*/usIndex = findHzIndex_16(pcStr);
//			usCh = * ( uint16_t * ) pStr;			
//			usCh = ( usCh << 8 ) + ( usCh >> 8 );		
 ...
 * @retval 无
 */
void LCD_DispString_EN_CH32( uint16_t Column , uint16_t Line, const uint8_t * pStr )
{
	uint16_t usCh;
...
void LCD_DrawLine(uint16_t Xpos, uint16_t Ypos, uint16_t Length, uint8_t Direction)
{
  ...
  uint32_t  Xaddress = 0;
  uint16_t Red_Value = 0, Green_Value = 0, Blue_Value = 0;
    // CurrentFrameBuffer + 3*(LCD_PIXEL_WIDTH*Ypos + Xpos);
    if (g_LcdDirection == 0)		/* 横屏 */
	{
		//index = (uint32_t)3*_usY * LCD_PIXEL_WIDTH + _usX;
//		index = (uint32_t)_usY*3 +( LCD_PIXEL_WIDTH *_usX)*3;
